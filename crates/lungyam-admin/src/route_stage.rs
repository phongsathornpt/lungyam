...
mod tests {
    ...
    #[test]
    fn existing_upstream_candidate_rejects_rename() {
        let config = test_config();
        let form = StageRouteForm {
            operation: "validate-upstream".to_owned(),
            original_name: String::new(),
            ...
            upstream_name: "renamed".to_owned(),
            endpoints: "127.0.0.1:3000".to_owned(),
            connect_timeout: "Default".to_owned(),
            read_timeout: "Default".to_owned(),
            write_timeout: "Default".to_owned(),
            health_check_interval: "5 s".to_owned(),
        };

        assert!(candidate_upstream_config(&config, &form).is_err());
    }
