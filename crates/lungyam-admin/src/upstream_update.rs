...
pub(super) struct StageUpstreamUpdateForm {
    #[serde(default)]
    csrf_token: String,
...
pub(super) async fn stage_update(
    ...
    Form(form): Form<StageUpstreamUpdateForm>,
) -> Response {
    ...
    match FileConfigLifecycle::new(config_path).stage(
        ...
        Some(format!("stage upstream update '{upstream_name}'")),
    ) {
        Ok(metadata) => render_success(metadata.revision),
        Err(error) => {
            log::error!("failed to stage upstream update: {error}");
            render_error(
...
fn candidate_config(config: &Config, form: &StageUpstreamUpdateForm) -> Result<Config, String> {
    let original_name = form.upstream_original_name.trim();
    let upstream_name = form.upstream_name.trim();
    if original_name.is_empty() || upstream_name.is_empty() {
...
fn render_error(status: StatusCode, message: &str) -> Response {
    log::warn!("upstream update staging rejected: {message}");
    (
        status,
        Html(
...
mod tests {
    ...
    use super::{StageUpstreamUpdateForm, candidate_config, stage_update};
    use crate::{AdminState, security};

    static TEST_SEQUENCE: AtomicU64 = AtomicU64::new(1);

    #[test]
    fn candidate_update_rejects_rename_and_preserves_route_reference() {
        let config = test_config(false);
        let mut renamed = form();
        renamed.upstream_name = "renamed".to_owned();
        assert!(candidate_config(&config, &renamed).is_err());

        let candidate = candidate_config(&config, &form()).expect("valid candidate");
        let api = &candidate.upstreams["api"];
        assert_eq!(api.endpoints.len(), 2);
        assert_eq!(api.connect_timeout_ms, Some(2500));
        assert_eq!(api.read_timeout_ms, Some(12000));
        assert_eq!(api.write_timeout_ms, Some(15000));
        assert_eq!(api.health_check_interval_seconds, 7);
        assert_eq!(candidate.routes[0].upstream, "api");
    }

    #[test]
    fn handler_stages_update_without_replacing_active_config() {
        ...
        runtime.block_on(async {
            ...
            let response = stage_update(State(state), Form(request)).await;
            ...
            let api = &snapshot.config.upstreams["api"];
            assert_eq!(api.endpoints.len(), 2);
            assert_eq!(api.read_timeout_ms, Some(12000));
            assert_eq!(snapshot.config.routes[0].upstream, "api");
    ...
    fn handler_rejects_read_only_and_invalid_csrf_before_staging() {
        ...
        runtime.block_on(async {
            ...
            let mut request = form();
            request.csrf_token = security::csrf_token().expose().to_owned();
            let response = stage_update(State(read_only_state), Form(request)).await;
            ...
            let response = stage_update(State(writable_state), Form(request)).await;
            assert_eq!(response.status(), StatusCode::FORBIDDEN);
    ...
    fn form() -> StageUpstreamUpdateForm {
        StageUpstreamUpdateForm {
            csrf_token: String::new(),
            upstream_original_name: "api".to_owned(),
            upstream_name: "api".to_owned(),
            endpoints: "127.0.0.1:4000\n127.0.0.1:4001".to_owned(),
            connect_timeout: "2500".to_owned(),
            read_timeout: "12000".to_owned(),
    ...
    fn test_directory() -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "lungyam-admin-upstream-update-{}-{sequence}",
            std::process::id()
        ))
    }
